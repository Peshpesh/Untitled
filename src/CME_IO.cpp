...
CME_IO CME_IO::IOControl;
int CME_IO::symbolnum = 0;
...
char* CME_IO::OnSave(SDL_Renderer* renderer, SDL_Texture* map_ui, SDL_Texture* font)
{
	Running = true;
	SDL_Event Event;
	while (Running)
...
bool CME_IO::OnLoad(SDL_Renderer* renderer, SDL_Texture* map_ui, SDL_Texture* font, char* &setpath)
{
	Running = true;
	SDL_Event Event;
	while (Running)
...
void CME_IO::OnEvent(SDL_Event* Event)
{
	CEvent::OnEvent(Event);
}

void CME_IO::OnKeyDown(SDL_Keycode sym, Uint16 mod)
{
	if (symbolnum < 15)
	{
		switch (sym)
